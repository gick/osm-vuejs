<template>
	<v-ons-page @show="resetBadge" v-show="$store.state.user.id">
		<v-ons-card>
      		<v-ons-row>
    	...
    	<v-ons-card>
    		<v-ons-row>
    			<v-ons-col width="20%" v-for="(trophy,index) in trophies">
    				<center>
    					<v-ons-icon v-if="trophy.obtenu" icon="fa-trophy" style="color:#f4c242" @click="displayTrophy(index)"></v-ons-icon>
    					<v-ons-icon v-else icon="fa-trophy" style="opacity:0.2" @click="displayTrophy(index)"></v-ons-icon>
    				</center>	
    			</v-ons-col>
    		</v-ons-row>
    	</v-ons-card>
    	...
    	<v-card>
	    		<v-ons-card v-for="trophy in trophies">
	    			<img v-if="trophy.obtenu" :src="'./trophees/' + trophy.path">
	    			<img v-else :src="'./trophees/' + trophy.path" style="opacity:0.2">
	    			<br>
